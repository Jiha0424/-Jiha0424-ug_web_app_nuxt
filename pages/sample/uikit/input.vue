<script setup>
import CountryService from '~/services/ConutryService';
...
onMounted(() => {
  countryService.getCountry().then((data) => (autoValue.value = data));
  nodeService.getTreeNode().then((data) => (treeSelectNodes.value = data));
});
  ...
  setTimeout(() => {
    if (!event.query.trim().length) {
      ...
      autoFilteredValue.value = autoValue.value.filter((country) => {
        return country.name.toLowerCase().startWith(event.query.toLowerCase());
      });
    }
  }, 250);
};
